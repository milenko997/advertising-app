            ...
            <div id="page-admin-messages" className="py-8">
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="flex items-center justify-between mb-6">
                        <h1 className="text-xl font-bold text-gray-900 dark:text-neutral-100">Kontakt poruke</h1>
                        {unreadCount > 0 && (
                    ...
                    {messageList.length === 0 ? (
                        <div className="text-center py-16 bg-white dark:bg-neutral-800 rounded-xl border border-gray-200 dark:border-neutral-700">
                            ...
                            <p className="text-gray-500 dark:text-neutral-400">Nema poruka.</p>
                        </div>
                    ) : (
                        <div className="space-y-3">
                            {messageList.map(msg => (
                                <div
                                    key={msg.id}
                                    className={`bg-white dark:bg-neutral-800 rounded-xl border shadow-sm overflow-hidden transition-colors ${
                                        msg.read ? 'border-gray-200 dark:border-neutral-700' : 'border-orange-200 dark:border-orange-800'
                                    }`}
                                >
                                    {/* Clickable header */}
                                    <button
                                        onClick={() => toggleExpand(msg.id)}
                                        className="w-full text-left p-5 hover:bg-gray-50 dark:hover:bg-neutral-700 transition-colors"
                                    >
                                        <div className="flex items-start justify-between gap-4">
                                            <div className="flex items-center gap-2.5">
                                                ...
                                                <div>
                                                    <span className={`text-sm ${msg.read ? 'font-medium text-gray-700 dark:text-neutral-300' : 'font-semibold text-gray-900 dark:text-neutral-100'}`}>
                                                        {msg.name}
                                                    </span>
                                                    <p className="text-xs text-orange-500 mt-0.5">{msg.email}</p>
                                                    <p className="text-xs text-gray-400 dark:text-neutral-500 mt-0.5">{msg.subject}</p>
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-3 shrink-0">
                                                <div className="text-right">
                                                    <p className="text-xs text-gray-400 dark:text-neutral-500">{msg.date}</p>
                                                    <p className="text-xs text-gray-400 dark:text-neutral-500">{msg.time}</p>
                                                </div>
                                                <svg
                                                    className={`w-4 h-4 text-gray-400 dark:text-neutral-500 transition-transform ${expanded === msg.id ? 'rotate-180' : ''}`}
                                    ...
                                    {expanded === msg.id && (
                                        <div className="px-5 pb-5 border-t border-gray-100 dark:border-neutral-700">
                                            <p className="text-sm text-gray-700 dark:text-neutral-300 leading-relaxed whitespace-pre-wrap pt-4">{msg.message}</p>
                                            <div className="flex items-center justify-between gap-3 mt-4 pt-3 border-t border-gray-100 dark:border-neutral-700">
                                                <a
